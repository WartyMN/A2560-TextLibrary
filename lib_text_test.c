...
//  	the_message = General_StrlcpyWithAlloc((char*)"THISISAREALLYBIGWORDBIGGERTHANANYYOUCANTHINK_OF_OR_AT_LEAST_I_THINK_SO", 80*60+1);
  	...
  	mu_assert(Text_DrawStringInBox(ID_CHANNEL_A, 2, 5, 67, 50, the_message, FG_COLOR_OLIVE, BG_COLOR_BROWN) == true, "Text_DrawStringInBox failed" );
  	mu_assert(Text_DrawStringInBox(ID_CHANNEL_B, 20, 20, 40, 40, the_message, FG_COLOR_BLACK_GREEN, BG_COLOR_BROWN) == true, "Text_DrawStringInBox failed" );
}
