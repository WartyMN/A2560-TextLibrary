...
MU_TEST(text_test_block_copy)
{
	...
	mu_assert( (buffer1 = (char*)calloc(the_system->screen_[ID_CHANNEL_A]->text_mem_cols_ * the_system->screen_[ID_CHANNEL_A]->text_mem_rows_, sizeof(char)) ) != NULL, "could not alloc space for screen buffer 1");
 	mu_assert( (buffer2 = (char*)calloc(the_system->screen_[ID_CHANNEL_A]->text_mem_cols_ * the_system->screen_[ID_CHANNEL_A]->text_mem_rows_, sizeof(char)) ) != NULL, "could not alloc space for screen buffer 2");

	
	// copy text on channel B, to off-screen buffer 1	
	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_B], buffer1, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy chan B char to buffer 1" );
 
 	// copy text on channel A, to off-screen buffer 2
 	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_A], buffer2, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy chan A char to buffer 2" );
 	
 	// copy text in offscreen buffer 1, to channel A
 	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_A], buffer1, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy buffer1 to chan A char" );
 
 	// copy text in offscreen buffer 2, to channel B
	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_B], buffer2, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy buffer2 to chan B char" );
 	
 	// copy attr on channel B, to off-screen buffer 1	
	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_B], buffer1, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy chan B attr to buffer 1" );
 
 	// copy attr on channel A, to off-screen buffer 2
	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_A], buffer2, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy chan A attr to buffer 2" );
 	
 	// copy attr in offscreen buffer 1, to channel A
	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_A], buffer1, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy buffer1 to chan A attr" );
 
 	// copy attr in offscreen buffer 2, to channel B
// 	mu_assert( Text_CopyScreen(the_system->screen_[ID_CHANNEL_B], buffer2, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy buffer2 to chan B attr" );


// 	// copy text on channel B, to off-screen buffer 1	
// 	mu_assert( Text_CopyCharMemFromScreen(the_system->screen_[ID_CHANNEL_B], buffer1) == true, "Could not copy chan B char to buffer 1" );
// 
// 	// copy text on channel A, to off-screen buffer 2
// 	mu_assert( Text_CopyCharMemFromScreen(the_system->screen_[ID_CHANNEL_A], buffer2) == true, "Could not copy chan A char to buffer 2" );
// 	
// 	// copy text in offscreen buffer 1, to channel A
// 	mu_assert( Text_CopyCharMemToScreen(the_system->screen_[ID_CHANNEL_A], buffer1) == true, "Could not copy buffer1 to chan A char" );
// 
// 	// copy text in offscreen buffer 2, to channel B
// 	mu_assert( Text_CopyCharMemToScreen(the_system->screen_[ID_CHANNEL_B], buffer2) == true, "Could not copy buffer2 to chan B char" );
// 	
// 	// copy attr on channel B, to off-screen buffer 1	
// 	mu_assert( Text_CopyAttrMemFromScreen(the_system->screen_[ID_CHANNEL_B], buffer1) == true, "Could not copy chan B attr to buffer 1" );
// 
// 	// copy attr on channel A, to off-screen buffer 2
// 	mu_assert( Text_CopyAttrMemFromScreen(the_system->screen_[ID_CHANNEL_A], buffer2) == true, "Could not copy chan A attr to buffer 2" );
// 	
// 	// copy attr in offscreen buffer 1, to channel A
// 	mu_assert( Text_CopyAttrMemToScreen(the_system->screen_[ID_CHANNEL_A], buffer1) == true, "Could not copy buffer1 to chan A attr" );
// 
// 	// copy attr in offscreen buffer 2, to channel B
// 	mu_assert( Text_CopyAttrMemToScreen(the_system->screen_[ID_CHANNEL_B], buffer2) == true, "Could not copy buffer2 to chan B attr" );
...
MU_TEST(text_test_block_copy_box)
{
	...
	// copy text on channel B, to off-screen buffer 1	
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_B], buffer1, x, y, x+h_line_len, y+v_line_len, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy box of chan B char to buffer 1" );

	// copy text on channel A, to off-screen buffer 2
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_A], buffer2, 0, 0, 71, 8, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy box of chan A char to buffer 2" );
	
	// copy text in offscreen buffer 1, to channel A
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_A], buffer1, x, y, x+h_line_len, y+v_line_len, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_CHAR == true),  "Could not copy box of buffer1 to chan A char" );

	// copy text in offscreen buffer 2, to channel B
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_B], buffer2, 0, 0, 71, 8, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_CHAR == true), "Could not copy box of buffer2 to chan B char" );
	
	// copy attr on channel B, to off-screen buffer 1	
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_B], buffer1, x, y, x+h_line_len, y+v_line_len, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy box of chan B attr to buffer 1" );

	// copy attr on channel A, to off-screen buffer 2
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_A], buffer2, 0, 0, 71, 8, SCREEN_COPY_FROM_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy box of chan A attr to buffer 2" );
	
	// copy attr in offscreen buffer 1, to channel A
	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_A], buffer1, x, y, x+h_line_len, y+v_line_len, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy box of buffer1 to chan A attr" );

	// copy attr in offscreen buffer 2, to channel B
 	mu_assert( Text_CopyMemBox(the_system->screen_[ID_CHANNEL_B], buffer2, 0, 0, 71, 8, SCREEN_COPY_TO_SCREEN, SCREEN_FOR_TEXT_ATTR == true), "Could not copy box of buffer2 to chan B attr" );
...
{
	mu_assert( Text_FillCharMem(the_system->screen_[ID_CHANNEL_A], 'Z'), "Could not fill character memory in channel A" );
	mu_assert( Text_FillCharMem(the_system->screen_[ID_CHANNEL_B], 4), "Could not fill character memory in channel B" );
...
{
	mu_assert( Text_FillAttrMem(the_system->screen_[ID_CHANNEL_A], 127), "Could not fill attribute memory in channel A" );
	mu_assert( Text_FillAttrMem(the_system->screen_[ID_CHANNEL_B], 148), "Could not fill attribute memory in channel B" );
...
{
	// good values
	mu_assert( Text_FillBoxSlow(the_system->screen_[ID_CHANNEL_A], 0, 6, 15, 8, CH_CHECKERED1, COLOR_BLACK, BG_COLOR_WHITE, CHAR_AND_ATTR) == true, "Text_FillBoxSlow failed" );
	mu_assert( Text_FillBoxSlow(the_system->screen_[ID_CHANNEL_A], 21, 5, 39, 7, CH_CHECKERED2, COLOR_DK_RED, COLOR_RED, CHAR_AND_ATTR) == true, "Text_FillBoxSlow failed" );
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_A], 3, 6, 67, 50, CH_CHECKERED3, COLOR_GREEN, COLOR_DK_GREEN) == true, "Text_FillBox failed" );
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_B], 21, 21, 40, 40, CH_CHECKERED3, COLOR_YELLOW, COLOR_DK_YELLOW) == true, "Text_FillBox failed" );
	...
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_B], -67, 6, 72, 30, CH_CHECKERED3, COLOR_BLUE, COLOR_DK_BLUE) == false, "Text_FillBoxSlow accepted an illegal x coord" );
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_B], 450000, 6, 72, 30, CH_CHECKERED3, COLOR_BLUE, COLOR_DK_BLUE) == false, "Text_FillBoxSlow accepted an illegal x coord" );
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_B], 5, -6, 72, 30, CH_CHECKERED3, COLOR_BLUE, COLOR_DK_BLUE) == false, "Text_FillBoxSlow accepted an illegal y coord" );
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_B], 5, 6000, 72, 30, CH_CHECKERED3, COLOR_BLUE, COLOR_DK_BLUE) == false, "Text_FillBoxSlow accepted an illegal y coord" );
}
...
MU_TEST(text_test_invert_box)
{
	...
	for (i = 0; i < 999; i++)
	{
		mu_assert( Text_InvertBox(the_system->screen_[ID_CHANNEL_B], 0, 6, 15, 8), "Could not invert color of a box" );
	}
	
	mu_assert( Text_InvertBox(the_system->screen_[ID_CHANNEL_B], 50, 13, 71, 16), "Could not invert color of a box" );

	// bad values
	mu_assert( Text_InvertBox(NULL, 50, 13, 71, 16) == false, "Text_InvertBox accepted an illegal screen ID" );
	mu_assert( Text_InvertBox(the_system->screen_[ID_CHANNEL_B], 50, 13, 1500, 16) == false, "Text_InvertBox accepted illegal rect coordinates" );
	mu_assert( Text_InvertBox(the_system->screen_[ID_CHANNEL_B], 71, 16, 50, 10) == false, "Text_InvertBox accepted illegal rect coordinates" );
...
{
	mu_assert( Text_UpdateFontData(the_system->screen_[ID_CHANNEL_A], (char*)0x000000), "Could not replace font data for channel A" );
	mu_assert( Text_UpdateFontData(the_system->screen_[ID_CHANNEL_B], (char*)0x000000), "Could not replace font data for channel B" );
...
{
	mu_assert( Text_ShowFontChars(the_system->screen_[ID_CHANNEL_A], 10), "Could not show font chars for channel A" );
	mu_assert( Text_ShowFontChars(the_system->screen_[ID_CHANNEL_B], 10), "Could not show font chars for channel B" );
...
MU_TEST(text_test_char_placement)
{
	...
	for (y = 4; y < 40; y = y+2)
	{
		for (x = 0; x < 50; x++)
		{
			mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], x, y, 'X'), "text char placement failed" );
		}
	}
	
	// bad values
 	mu_assert( Text_SetCharAtXY(NULL, 0, 0, 'X') == false, "Text_SetCharAtXY accepted illegal screen ID" );
	mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], -1, 3, 'b') == false, "Text_SetCharAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], -1, -1, 'c') == false, "Text_SetCharAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], 0, -1, 'd') == false, "Text_SetCharAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], 500, 4, 'e') == false, "Text_SetCharAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], 500, 500, 'f') == false, "Text_SetCharAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], 0, 500, 'g') == false, "Text_SetCharAtXY accepted illegal coordinates" );
}
...
// same story here: 4 or so works ok, add more, and it's likely to crash. 
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 0, 4, 33, FG_COLOR_BLACK, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 1, 4, 34, FG_COLOR_DK_RED, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 2, 4, 35, FG_COLOR_GREEN, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 3, 4, 36, FG_COLOR_BLUE, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 4, 4, 37, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 5, 4, 38, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 6, 4, 39, FG_COLOR_VIOLET, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 7, 4, 40, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 8, 4, 41, COLOR_BLACK, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 9, 4, 42, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 10, 4, 43, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 11, 4, 44, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 12, 4, 45, FG_COLOR_DK_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 13, 4, 46, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 14, 4, 47, FG_COLOR_WHITE, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 15, 4, 48, COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 0, 5, 33, FG_COLOR_WHITE, BG_COLOR_BLACK) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 1, 5, 34, FG_COLOR_WHITE, BG_COLOR_DK_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 2, 5, 35, FG_COLOR_WHITE, BG_COLOR_GREEN) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 3, 5, 36, FG_COLOR_WHITE, BG_COLOR_BLUE) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 4, 4, 37, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 5, 5, 38, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 6, 5, 39, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 7, 5, 40, FG_COLOR_WHITE, BG_COLOR_DK_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 8, 5, 41, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 9, 5, 42, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 10, 5, 43, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 11, 5, 44, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 12, 5, 45, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 13, 5, 46, FG_COLOR_WHITE, COLOR_RED) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 14, 5, 47, FG_COLOR_WHITE, BG_COLOR_LT_GRAY) == true, "Text_SetCharAndColorAtXY failed" );
 	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_B], 15, 5, 48, FG_COLOR_WHITE, BG_COLOR_WHITE) == true, "Text_SetCharAndColorAtXY failed" );
	...
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], -1, 3, 'b', FG_COLOR_WHITE, BG_COLOR_WHITE) == false, "Text_SetCharAndColorAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], -1, -1, 'c', FG_COLOR_WHITE, BG_COLOR_WHITE) == false, "Text_SetCharAndColorAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 0, -1, 'd', FG_COLOR_WHITE, BG_COLOR_WHITE) == false, "Text_SetCharAndColorAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 500, 4, 'e', FG_COLOR_WHITE, BG_COLOR_WHITE) == false, "Text_SetCharAndColorAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 500, 500, 'f', FG_COLOR_WHITE, BG_COLOR_WHITE) == false, "Text_SetCharAndColorAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 0, 500, 'g', FG_COLOR_WHITE, BG_COLOR_WHITE) == false, "Text_SetCharAndColorAtXY accepted illegal coordinates" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 1, 1, 'h', -1, 1) == false, "Text_SetCharAndColorAtXY accepted illegal color value" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 1, 1, 'i', 17, 1) == false, "Text_SetCharAndColorAtXY accepted illegal color value" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 1, 1, 'j', 1, -54) == false, "Text_SetCharAndColorAtXY accepted illegal color value" );
	mu_assert( Text_SetCharAndColorAtXY(the_system->screen_[ID_CHANNEL_A], 1, 1, 'k', 1, 23) == false, "Text_SetCharAndColorAtXY accepted illegal color value" );
}
...
MU_TEST(text_test_char_and_attr_reading)
{
	...
	Text_SetAttrAtXY(the_system->screen_[ID_CHANNEL_A], x, y, FG_COLOR_WHITE, BG_COLOR_BLACK);
	Text_SetAttrAtXY(the_system->screen_[ID_CHANNEL_A], x+1, y, FG_COLOR_VIOLET, BG_COLOR_DK_GRAY);
	Text_SetAttrAtXY(the_system->screen_[ID_CHANNEL_A], x+2, y, COLOR_ORANGE, BG_COLOR_WHITE);
	Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], x, y, CH_DIAMOND);
	Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], x+1, y, CH_CLUB);
	Text_SetCharAtXY(the_system->screen_[ID_CHANNEL_A], x+2, y, CH_SPADE);
	
	mu_assert( (the_color = Text_GetForeColorAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == FG_COLOR_WHITE, "Text_GetForeColorAtXY failed" );
	mu_assert( (the_color = Text_GetBackColorAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == BG_COLOR_BLACK, "Text_GetBackColorAtXY failed" );
	
	x++;
	mu_assert( (the_color = Text_GetForeColorAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == FG_COLOR_VIOLET, "Text_GetForeColorAtXY failed" );
	mu_assert( (the_color = Text_GetBackColorAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == BG_COLOR_DK_GRAY, "Text_GetBackColorAtXY failed" );

	x++;
	mu_assert( (the_color = Text_GetAttrAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == the_attribute_value, "Text_GetAttrAtXY failed");
	
	x = 0;
	mu_assert( (the_char = Text_GetCharAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == CH_DIAMOND, "Text_GetCharAtXY failed");	
	x++;
	mu_assert( (the_char = Text_GetCharAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == CH_CLUB, "Text_GetCharAtXY failed");	
	x++;
	mu_assert( (the_char = Text_GetCharAtXY(the_system->screen_[ID_CHANNEL_A], x, y)) == CH_SPADE, "Text_GetCharAtXY failed");	
}
...
MU_TEST(text_test_line_drawing)
{
	...
	line_len = 20;
	the_char = CH_WALL_H;	
 	mu_assert( Text_DrawHLine(the_system->screen_[ID_CHANNEL_A], x, y, line_len, the_char, FG_COLOR_GREEN, BG_COLOR_BLACK, CHAR_ONLY) == true, "Text_DrawHLine failed" );
	
	y = 8;
 	mu_assert( Text_DrawHLine(the_system->screen_[ID_CHANNEL_A], x, y, line_len, the_char, FG_COLOR_GREEN, BG_COLOR_BLACK, CHAR_AND_ATTR) == true, "Text_DrawHLine failed" );
	
	y = 4;
	line_len = 4;
	the_char = CH_WALL_V;	
 	mu_assert( Text_DrawVLine(the_system->screen_[ID_CHANNEL_A], x, y, line_len, the_char, BG_COLOR_YELLOW, BG_COLOR_BLACK, ATTR_ONLY) == true, "Text_DrawVLine failed" );

	x = x + 20;	
 	mu_assert( Text_DrawVLine(the_system->screen_[ID_CHANNEL_A], x, y, line_len, the_char, BG_COLOR_YELLOW, BG_COLOR_BLACK, CHAR_AND_ATTR) == true, "Text_DrawVLine failed" );
	...
	// bad values
 	mu_assert( Text_DrawVLine(NULL, x, y, line_len, the_char, COLOR_DK_RED, COLOR_RED, CHAR_AND_ATTR) == false, "Text_DrawVLine accepted illegal screen ID" );
  	mu_assert( Text_DrawVLine(the_system->screen_[ID_CHANNEL_B], -1, y, line_len, the_char, COLOR_DK_RED, COLOR_RED, CHAR_AND_ATTR) == false, "Text_DrawVLine accepted illegal x coord" );
 	mu_assert( Text_DrawVLine(the_system->screen_[ID_CHANNEL_B], x, 425, line_len, the_char, COLOR_DK_RED, COLOR_RED, CHAR_AND_ATTR) == false, "Text_DrawVLine accepted illegal y coord" );
...
MU_TEST(text_test_basic_box_coords)
{
	...
	// good values	
 	mu_assert( Text_DrawBoxCoords(the_system->screen_[ID_CHANNEL_A], x, y, x + h_line_len, y + v_line_len, the_char, FG_COLOR_LT_GRAY, BG_COLOR_DK_GRAY, CHAR_AND_ATTR) == true, "Text_DrawBoxCoords failed" );
...
MU_TEST(text_test_basic_box_hw)
{
	...
	// good values	
 	mu_assert(Text_DrawBox(the_system->screen_[ID_CHANNEL_A], x, y, h_line_len, v_line_len, the_char, FG_COLOR_CYAN, BG_COLOR_DK_CYAN, CHAR_AND_ATTR) == true, "Text_DrawBox failed" );

	x += 7;
	y += 2;
 	mu_assert(Text_DrawBox(the_system->screen_[ID_CHANNEL_A], x, y, h_line_len-2, v_line_len+5, --the_char, FG_COLOR_CYAN, BG_COLOR_DK_CYAN, CHAR_AND_ATTR) == true, "Text_DrawBox failed" );


	// bad values
 	mu_assert(Text_DrawBox(the_system->screen_[ID_CHANNEL_B], -10, y, h_line_len, v_line_len, the_char, FG_COLOR_CYAN, BG_COLOR_DK_CYAN, CHAR_AND_ATTR) == false, "Text_DrawBox accepted illegal x coord" );
}
...
//  	the_message = General_StrlcpyWithAlloc((char*)"THISISAREALLYBIGWORDBIGGERTHANANYYOUCANTHINK_OF_OR_AT_LEAST_I_THINK_SO", 80*60+1);
	...
	x2 = 98;
	y2 = 73;
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_A], x1+1, y1+1, x2-1, y2-1, CH_CHECKERED1, BG_COLOR_CYAN, BG_COLOR_DK_BLUE) == true, "Text_FillBox failed" );
 	mu_assert(Text_DrawBoxCoordsFancy(the_system->screen_[ID_CHANNEL_A], x1, y1, x2, y2, FG_COLOR_LT_GRAY, BG_COLOR_BLACK) == true, "Text_DrawBoxCoordsFancy failed" );

	x1 = 3;
	y1 = 3;
	x2 = 97;
	y2 = 72;
  	mu_assert(Text_DrawStringInBox(the_system->screen_[ID_CHANNEL_A], x1, y1, x2, y2, the_message, FG_COLOR_WHITE, BG_COLOR_BLACK, NULL) != NULL, "Text_DrawStringInBox failed" );
	...
	mu_assert( Text_FillBox(the_system->screen_[ID_CHANNEL_A], x1+1, y1+1, x2-1, y2-1, CH_CHECKERED3, FG_COLOR_LT_GRAY, BG_COLOR_WHITE) == true, "Text_FillBox failed" );
 	mu_assert(Text_DrawBoxCoordsFancy(the_system->screen_[ID_CHANNEL_A], x1, y1, x2, y2, FG_COLOR_LT_GRAY, BG_COLOR_DK_GRAY) == true, "Text_DrawBoxCoordsFancy failed" );

	x1 = 13;
	y1 = 5;
	x2 = 67;
	y2 = 50;
  	mu_assert(Text_DrawStringInBox(the_system->screen_[ID_CHANNEL_A], x1, y1, x2, y2, the_message, FG_COLOR_BLACK, BG_COLOR_WHITE, NULL) != NULL, "Text_DrawStringInBox failed" );
...
MU_TEST(text_test_draw_string)
{
	...
	mu_assert_string_eq("this is a string", (char*)the_message);
 	mu_assert(Text_DrawStringAtXY(the_system->screen_[ID_CHANNEL_B], 0, 5, the_message, FG_COLOR_YELLOW, BG_COLOR_DK_BLUE) == true, "Text_DrawStringAtXY failed" );
 	mu_assert(Text_DrawStringAtXY(the_system->screen_[ID_CHANNEL_B], 67, 4, the_message, FG_COLOR_DK_BLUE, BG_COLOR_YELLOW) == true, "Text_DrawStringAtXY failed" );
		
	// bad values
 	mu_assert(Text_DrawStringAtXY(the_system->screen_[ID_CHANNEL_B], -1, 0, the_message, FG_COLOR_DK_BLUE, BG_COLOR_YELLOW) == false, "Text_DrawBoxCoordsFancy accepted illegal x coord" );
}
...
//  	the_message = General_StrlcpyWithAlloc((char*)"THISISAREALLYBIGWORDBIGGERTHANANYYOUCANTHINK_OF_OR_AT_LEAST_I_THINK_SO", 80*60+1);
  	...
  	mu_assert(Text_DrawStringInBox(the_system->screen_[ID_CHANNEL_A], 3, 6, 67, 50, the_message, FG_COLOR_BLACK, BG_COLOR_WHITE, NULL) != NULL, "Text_DrawStringInBox failed" );
  	mu_assert(Text_DrawStringInBox(the_system->screen_[ID_CHANNEL_B], 21, 21, 40, 40, the_message, FG_COLOR_WHITE, BG_COLOR_BLACK, NULL) != NULL, "Text_DrawStringInBox failed" );
}
...
//   	mu_assert(Text_UpdateFontData(the_system->screen_[ID_CHANNEL_A], (char*)the_new_font_data) == true, "Failed to update font data for Channel A" );
  	mu_assert(Text_UpdateFontData(the_system->screen_[ID_CHANNEL_B], (char*)the_new_font_data) == true, "Failed to update font data for Channel B" );
}
...
MU_TEST(text_test_hline_speed)
{
	...
	for (j = 0; j < num_cycles; j++)
	{
		...
		{
			mu_assert( Text_DrawHLineSlow(the_system->screen_[ID_CHANNEL_A], x, y + i, line_len, the_char, FG_COLOR_GREEN, BG_COLOR_BLACK, CHAR_ONLY) == true, "Text_DrawHLine failed" );
		}
	}
	...
	for (j = 0; j < num_cycles; j++)
	{
		for (i=0; i < num_passes; i++)
		{
			mu_assert( Text_DrawHLine(the_system->screen_[ID_CHANNEL_A], x, y + i, line_len, the_char, FG_COLOR_RED, BG_COLOR_BLACK, CHAR_ONLY) == true, "Text_DrawHLine failed" );
		}
	}
	...
	for (j = 0; j < num_cycles; j++)
	{
		for (i=0; i < num_passes; i++)
		{
			mu_assert( Text_DrawHLineSlow(the_system->screen_[ID_CHANNEL_A], x, y + i, line_len, the_char, FG_COLOR_GREEN, BG_COLOR_BLACK, CHAR_AND_ATTR) == true, "Text_DrawHLine failed" );
		}
	}
	...
	for (j = 0; j < num_cycles; j++)
	{
		for (i=0; i < num_passes; i++)
		{
			mu_assert( Text_DrawHLine(the_system->screen_[ID_CHANNEL_A], x, y + i, line_len, the_char, FG_COLOR_RED, BG_COLOR_BLACK, CHAR_AND_ATTR) == true, "Text_DrawHLine failed" );
		}
	}
